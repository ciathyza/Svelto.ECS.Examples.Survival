...
public class SpawningData : MonoBehaviour
{
	...
	void Awake()
	{
		if (serializedSpawnDataOnce == false) SerializeSpawnData();
		if (serializedAttackDataOnce == false) SerializeAttackData();
	}
	...
	public void SerializeSpawnData()
	{
		...
		for (int i = 0; i < data.Length; i++) spawningdata[i] = new JSonEnemySpawnData(data[i].spawnData);
	...
	public void SerializeAttackData()
	{
		...
		for (int i = 0; i < data.Length; i++) attackData[i] = new JSonEnemyAttackData(data[i].attackData);

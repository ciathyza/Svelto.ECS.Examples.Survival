...
namespace Svelto.ECS.Example.Survive.HUD
{
	...
	public class HUDEngine : IQueryingEntityViewEngine, IStep<DamageInfo, DamageCondition>
	{
		...
		void OnDeadEvent()
		{
			int hudEntityViewsCount;
			var hudEntityViews = entityViewsDB.QueryEntities<HUDEntityView>(out hudEntityViewsCount);
			for (int i = 0; i < hudEntityViewsCount; i++) hudEntityViews[i].healthSliderComponent.value = 0;
		...
		IEnumerator RestartLevelAfterFewSeconds()
		{
			...
			int hudEntityViewsCount;
			var hudEntityViews = entityViewsDB.QueryEntities<HUDEntityView>(out hudEntityViewsCount);
			for (int i = 0; i < hudEntityViewsCount; i++) hudEntityViews[i].HUDAnimator.playAnimation = "GameOver";
		...
		{
			if (condition == DamageCondition.Damage) OnDamageEvent(token);
			else if (condition == DamageCondition.Dead) OnDeadEvent();
		}

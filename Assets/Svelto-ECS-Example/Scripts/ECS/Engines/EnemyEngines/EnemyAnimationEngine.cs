namespace Svelto.ECS.Example.Survive.Enemies
{
	public class EnemyAnimationEngine : IQueryingEntityViewEngine, IStep<DamageInfo, DamageCondition>
	{
		...
		void TriggerTargetDeathAnimation()
		{
			...
			for (int i = 0; i < count; i++) entity[i].animationComponent.playAnimation = "PlayerDead";
		}

		void TriggerDeathAnimation(EGID targetID)
		{
			entityViewsDB.ExecuteOnEntity(targetID,
				(ref EnemyEntityViewStruct entity) => { entity.animationComponent.playAnimation = "Dead"; });
		}
		...
		public void Step(ref DamageInfo token, DamageCondition condition)
		{
			...
			{
				//if enemy dies
				if (condition == DamageCondition.Dead) TriggerDeathAnimation(token.entityDamagedID);
				else
					//if enemy is damaged
					EntityDamaged(token);
			}
			else
			{
				...
				if (condition == DamageCondition.Dead) TriggerTargetDeathAnimation();
			}
		}
	}
}
